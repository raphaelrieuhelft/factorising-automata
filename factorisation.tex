...
Par ailleurs, étant donné une matrice $M$ à coefficients dans $\{0,1\}$, on pose $\sigma(M)$ l'entier obtenu en sommant ses lignes décalées (cf. Figure 2). Si $M$ représente une multiplication, $\sigma$ est par construction le produit. 
...
\begin{cases}
  \hfill x' \hfill & \text{ si}  \begin{cases}\text{ $x_{i,j}=1$ et $x_{i+1,j+1}=0$ }\\
    \text{ et} \begin{cases}
      \text {soit $(i+1,j+1)$ est une case dangereuse} \\
      \text {soit $\mathcal{B}(p) = 1$}
    \end{cases}
  \end{cases}
  \\
...
\begin{cases}
  \hfill x'' \hfill & \text {si} \begin{cases}\text{ $x_{i,j}=1$ et $x_{i-1,j}=x_{i,j+1}=0$} \\
    \text{ et} \begin{cases}
      \text {soit $(i-1,j)$ est une case dangereuse}\\
      \text {soit $(i,j+1)$ l'est}\\
      \text {soit $\mathcal{B}(p)=1$}
      \end{cases}
    \end{cases}
  \\
...
Les performances varient grandement selon la décomposition en facteurs premiers du nombre à factoriser. Ainsi, les nombres pour lesquelles les matrices solutions du problème comportent peu de $1$ semblent bien plus difficiles : alors que l'algorithme trouve un diviseur de $493$ ($17\times 29$) en $9\times 10^5$ pas de temps en moyenne, il lui faut $10^9$ étapes pour factoriser $362$ ($2\times 181$) ! Ceci est dû au fait que la densité de $1$ varie très peu au cours du temps (elle oscille autour d'une densité moyenne, et les solutions pour lesquelles la densité finale en est éloignée sont difficiles à trouver). 

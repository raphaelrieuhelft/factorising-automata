...
\begin{df*}
Pour toutes matrices $M$ et $N$, on a $M\sim N$ si et seulement si on peut passer de $M$ à $N$ en appliquant des transformations de $\Gamma$.
\end{df*}
...
On va montrer que pour tout $N$, il existe une unique matrice de la taille choisie dans l'algorithme telle que $\sigma = N$ et dont le nombre de coefficients égaux à $1$ est minimal. 
